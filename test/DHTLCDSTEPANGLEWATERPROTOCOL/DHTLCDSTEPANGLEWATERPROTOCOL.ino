...
void setup()
{
  ...
  pinMode(Relaypin,INPUT_PULLUP); // 릴레이핀을 출력으로 설정
...
void loop()
{
  ...
  angleVR=map(motVR,0,360,0,2048);
  {
    ...
    else if(digitalRead(Relaypin)==HIGH) // stop 스위치를 누르지 않으면
    {
  ...
  bsc=digitalRead(switchc); // 버튼의 상태 입력
  {
    ...
    else if(countc%2==1) //change count가 1이면 정회전
    {
      if(rain>=50)
      {
        angleVR=0;
    ...
    else if(countc%2==0) //change count가 0이면 역회전
    {
      myStepper.step(-angleVR);
    }
